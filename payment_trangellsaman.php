...
if (!class_exists ('checkHack')) {
	require_once( JPATH_PLUGINS . '/j2store/payment_trangellsaman/trangell_inputcheck.php');
}

class plgJ2StorePayment_trangellsaman extends J2StorePaymentPlugin
{
    ...
    function _prePayment( $data )
    {
        ...
        //============================================================================
		$vars->display_name = 'TrangellSaman';
		...
		if ($vars->samanmerchantId == null || $vars->samanmerchantId == ''){
			...
			$app->redirect($link, '<h2>لطفا تنظیمات درگاه سامان را بررسی کنید</h2>', $msgType='Error'); 
		}
		else {
			$vars->reservationNumber = time();
			$vars->totalAmount =  round($vars->orderpayment_amount,0);
			$vars->callBackUrl  = JRoute::_(JURI::root(). "index.php?option=com_j2store&view=checkout" ) .'&orderpayment_id='.$vars->orderpayment_id . '&orderpayment_type=' . $vars->orderpayment_type .'&task=confirmPayment' ;
	...
	function saveStatus($msg,$statCode,$customer_note,$emptyCart,$trackingCode,$orderpayment,$CardNumber){
		$html ='<br />';
		$html .='<strong>'.'TrangellSaman'.'</strong>';
		$html .='<br />';
